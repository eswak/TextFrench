...
void time_to_words2(int hours, int minutes, char* words, size_t length) {
  ...
  if (hours == 12) {
    remaining -= append_string(words, remaining, SPECIAL[2]);
  } else if (hours == 0) {
    remaining -= append_string(words, remaining, SPECIAL[3]);
  } else {
    remaining -= append_number(words, hours % 12);
    remaining -= append_string(words, remaining, " ");
    if (hours == 1 || hours == 13) {
      remaining -= append_string(words, remaining, SPECIAL[0]);
    } else {
      remaining -= append_string(words, remaining, SPECIAL[1]);
    }
  }
...
{
	char value[length];
	time_to_words2(hours, minutes, value, length);
